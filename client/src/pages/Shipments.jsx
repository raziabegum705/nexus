...
export default function Shipments() {
  ...
  useEffect(() => {
    ...
    socket.on('initial_data', ({ shipments: s }) => setShipments(s))
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: 'calc(100vh - 60px)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 700 }}>◈ Active Shipments</h1>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {['all', 'delayed', 'in_transit', 'on_time'].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{
              ...
              fontFamily: 'var(--font)',
              background: filter === f ? 'rgba(0,200,150,0.15)' : 'transparent',
              color: filter === f ? 'var(--green)' : 'var(--text2)',
              border: filter === f ? '1px solid rgba(0,200,150,0.3)' : '1px solid var(--border2)',
            }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                      <span style={{ fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--text3)' }}>{s.id}</span>
                    ...
                    <div style={{ fontSize: '15px', fontWeight: 600, marginBottom: '8px' }}>{s.name}</div>
                    <div style={{ display: 'flex', gap: '20px', fontSize: '13px', color: 'var(--text2)' }}>
          ...
          <div style={{ position: 'sticky', top: '80px', height: 'fit-content' }}>
            <div className="card" style={{ padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <span style={{ fontWeight: 600, fontSize: '15px' }}>🛣️ AI Reroute Options</span>
                <button onClick={() => { setSelected(null); setRoutes(null) }}
                  style={{ background: 'none', border: 'none', color: 'var(--text2)', cursor: 'pointer', fontSize: '18px' }}>×</button>
              </div>
              ...
              <div style={{
                ...
                <div style={{ color: 'var(--text2)' }}>{selected.origin.city} → {selected.destination.city}</div>
