...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('initial_data', ({ shipments: s, disruptions: d }) => {
      setShipments(s); setDisruptions(d)
    })
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: 'calc(100vh - 60px)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, letterSpacing: '-0.5px' }}>Mission Control</h1>
          ...
          <div key={i} className="card" style={{ padding: '18px 16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize: '11px', color: 'var(--text2)', marginTop: '4px' }}>{s.label}</div>
              </div>
